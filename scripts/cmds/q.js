...
module.exports = {
    ...
    onStart: async function({ bot, chatId, msg, args }) {
        ...
        try {
            ...
            const telegraphUrl = `https://apis-frnwot.onrender.com/telegraph?url=${encodeURIComponent(fileLink)}&senderId=4679926`;

            const response = await axios.get(telegraphUrl);
            const link = response.data.result.link;

            const imageUrl = `https://apis-frnwot.onrender.com/q?text=${encodeURIComponent(prompt)}&avatar=${link}&username=${username}`;
